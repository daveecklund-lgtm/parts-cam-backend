...
// Holds a Google refresh token permanently and exchanges it for fresh access
// tokens on request. This avoids Safari's silent-auth blocking entirely,
// since it never relies on browser cookies — just a direct server-to-Google
// call using a refresh token that Google issues once and doesn't expire
  ...
  try{
    ...
    if(!data.refresh_token){
      return res.status(500).send(
        'No refresh_token in response — this usually means you already granted access before. ' +
        'Go to myaccount.google.com/permissions, remove access for this app, then try /auth/start again. ' +
...
// Step 3 (called automatically by the app, forever): get a fresh access token
